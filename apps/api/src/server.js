...
async function auth(req, reply) {
  ...
  try {
    req.user = (await jwtVerify(token, secret, { issuer: "dashbv" })).payload;
  } catch {
...
app.get("/api/transactions", { preHandler: auth }, async (req) =>
  transactions(req.query),
);
app.get("/api/meta", { preHandler: auth }, async () => {
  const [funds, categories, lastImport] = await Promise.all([
    pool.query("SELECT DISTINCT fund FROM transactions ORDER BY fund"),
  ...
  return {
    funds: funds.rows.map((x) => x.fund),
    categories: categories.rows.map((x) => x.category),
    scopes: ["Área Geral", "Edifício Boa Vista"],
...
app.post(
  ...
  async (req, reply) => {
    if (!process.env.GROQ_API_KEY)
      return reply
        .code(503)
        .send({ error: "Agente Groq ainda não configurado" });
    const data = await dashboard(req.body || {});
    const cacheKey = crypto
      .createHash("sha256")
      .update(JSON.stringify({ q: req.body, data }))
      .digest("hex");
    const cached = await pool.query(
      "SELECT content FROM insight_cache WHERE cache_key=$1 AND created_at > now()-interval '12 hours'",
      [cacheKey],
    );
    ...
    if (!response.ok) {
      app.log.error({ status: response.status }, "Groq failed");
      return reply.code(502).send({ error: "Falha temporária no agente" });
    }
    const raw = await response.json();
    const content = JSON.parse(raw.choices[0].message.content);
    await pool.query(
      "INSERT INTO insight_cache(cache_key,content) VALUES($1,$2) ON CONFLICT(cache_key) DO UPDATE SET content=excluded.content,created_at=now()",
      [cacheKey, content],
    );
    return content;
  },
